<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="test.aspx.cs" Inherits="RM.Web.SysSetBase.hotelphoto.test" %>
...
<head id="Head1" runat="server">
    ...
    <script src="/Themes/Scripts/Function.js" type="text/javascript"></script>
...
//            $('.hotelphoto').on('click', '.add', function (ev) {
//                ev = ev || window.event;
//     
//                $(this).next().click();
//            });
                    ...
                    for (var i = 0; i < json.length; i++) {
                        ...
                        if (json[i].pic != undefined && json[i].pic != "") {
                            ...
                            for (var s = 0; s < json[i].pic.length; s++) {
                                html += "<div imgname='" + json[i].pic[s].IMGFILE + "'><img src='../../upload/photo/" + json[i].pic[s].IMGFILE + "' alt='Alternate Text' /><span class='gb'><i class='icon-close'></i></span> </div>";
                            }
            ...
            <div class="hotelphoto">
                ...
                <div class="hotelphotolist">
                    <div class="hotelphotolistt">
                        <span>房型图片</span><em>建议上传图片尺寸为4:3，每个房型最多上传5张图片，第1张图片同时作为缩略图和轮换图</em>
                    </div>
                    <div id="RoomPicture">
                    </div>
                </div>
                ...
                <div class="sharebottombtn" style="background-color: #fff;">

                       <a onclick="check()">提交</a>

               <%-- <input type="submit" name="btnSumit" value="提交"  onclick="check()"/>--%>
                </div>
            </div>
        </div>
    </div>
