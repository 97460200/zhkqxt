...
{
    public partial class ReceiptInfo : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                if (!string.IsNullOrEmpty(Request.QueryString["MemberId"]) && !string.IsNullOrEmpty(Request.QueryString["RechargeId"]))
                {
                    ...
                    Bind(AdminHotelid, MemberId, RechargeId);//加载用户信息
        ...
        private void Bind(string AdminHotelid, string MemberId, string RechargeId)
        {
            //查询充值记录
            StringBuilder sb_recharge = new StringBuilder();
            sb_recharge.Append(@"SELECT *  FROM CardMoney WHERE ID=@RechargeId ");
            SqlParam[] parm_recharge = new SqlParam[] {                  
                                     new SqlParam("@RechargeId", RechargeId)};
            DataTable dt_recharge = DataFactory.SqlDataBase().GetDataTableBySQL(sb_recharge, parm_recharge);
            if (dt_recharge != null && dt_recharge.Rows.Count > 0)
            {
                spTopUpMoney.InnerHtml= Convert.ToDouble(dt_recharge.Rows[0]["Money"]).ToString();
                TopUpTime.InnerHtml = Convert.ToDateTime(dt_recharge.Rows[0]["InDate"]).ToString("yyyy-MM-dd HH:mm"); 
            }
