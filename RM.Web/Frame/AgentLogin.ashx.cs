...
namespace RM.Web.Frame
{
    ...
    /// AgentLogin 的摘要说明
    /// </summary>
    public class AgentLogin : IHttpHandler
    {
        ...
        {
            string str = "";
            if (context.Request.Cookies["agent_cookie"] != null)
            {
                str = context.Request.Cookies["agent_cookie"]["UserName"] + "&" + context.Request.Cookies["agent_cookie"]["PassWord"];
            }
            if (str != "")
            {
        ...
        public void Menu(HttpContext context)
        {
            ...
            string UserId = context.Request.Cookies["dlagent_COOKIE"]["User_ID"];
            string strMenus = JsonHelper.DataTableToJson(sys_idao.GetMenuHtmlAgent(UserId), "MENU");
