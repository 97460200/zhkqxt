...
function iframeresize() {
    ...
    function resizeU() {
        ...
        $("#MainContent").height(divkuangH - MainContent_subtract + 12);
    }
}
/**安全退出**/
function IndexOut() {
    top.showConfirmMsg("是否确认退出系统？", function (r) {
        if (r) {
            getAjax("/Frame/Frame.ashx", "action=Outlogin", function (data) {
                window.location.href = '/Frame/login.aspx';
            });
        }
    });
}
function IndexOuts() {
    top.showConfirmMsg('　是否确认退出系统？　', function (r) {
        if (r) {
            getAjax("/Frame/Frame.ashx", "action=Outlogin", function (data) {
                window.location.href = '../Frame/adminLogin.htm';
            })
        }
    });
}
function IndexOutss() {
    top.showConfirmMsg('　是否确认退出系统？　', function (r) {
        if (r) {
            getAjax("/Frame/Frame.ashx", "action=Outlogin", function (data) {
                window.location.href = '../Frame/agentLogin.htm';
            })
        }
    });
}
...
function AddTabMenu(tabid, url, name, img, Isclose, IsReplace) {
    ...
    if (top.document.getElementById("tabs_" + tabid) == null) { //如果当前tabid存在直接显示已经打开的tab
        Loading(true);
        if (tabs_container.find('li').length > 15) {
            tabs_container.find('li:eq(1)').remove();
        }
        tabs_container.find('li').removeClass('selected');
        ContentPannel.find('iframe').hide();
        var tImg = "";
        if (img != "") {
            //tImg = "<img src=\"/Themes/Images/32/" + img + "\" width=\"20\" height=\"20\">";
        }
        if (Isclose != 'false') { //判断是否带关闭tab
            var cd = "<li id=\"tabs_" + tabid + "\" class='selected' win_close='true'><span title='" + name + "' onclick=\"javascript:AddTabMenu('" + tabid + "','" + url + "','" + name + "','" + img + "','false')\"><a href=\"javascript:;\">" + tImg + name + "</a></span><a class=\"win_close\" title=\"关闭当前窗口\" onclick=\"RemoveDiv('" + tabid + "')\"></a></li>";
            //            tabs_container.append("<li id=\"tabs_" + tabid + "\" class='selected' win_close='true'><span title='" + name + "' onclick=\"javascript:AddTabMenu('" + tabid + "','" + url + "','" + name + "','" + img + "','true')\"><a href=\"javascript:;\">" + tImg + name + "</a></span><a class=\"win_close\" title=\"关闭当前窗口\" onclick=\"RemoveDiv('" + tabid + "')\"></a></li>");

            $(tabs_container).find("li").eq(0).after(cd);

        } else {
            tabs_container.append("<li id=\"tabs_" + tabid + "\" class=\"selected\" onclick=\"javascript:AddTabMenu('" + tabid + "','" + url + "','" + name + "','" + img + "','false')\"><a>" + tImg + name + "</a></li>");
        }
