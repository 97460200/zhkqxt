...
$(document).ready(function () {
    ...
    //设置页面中，所有的上传文件的accept节点属性 （防止响应太慢）
    $("input[type='file']").attr("accept", "image/gif,image/jpeg,image/jpg,image/png,image/svgs")
})

function CheckAllLine2(a222) {
    var a22 = document.getElementsByName("checkbox");
    if (a222.className == "xuanxuanthis2") {
        a222.className = "xuanxuan2";
        for (var i = 0; i < a22.length; i++) {
            a22[i].className = "xuanxuan"
        }
    }
    else {
        a222.className = "xuanxuanthis2";
        for (var i = 0; i < a22.length; i++) {
            a22[i].className = "xuanxuanthis";
        }
    }
...
function CheckAllLine3(aa) {
    if (aa.className == "xuanxuanthis") {
        aa.className = "xuanxuan";
        var anchors = aa.parentNode.parentNode.childNodes;
        for (i = 0; i < anchors.length; i++) {
            anchors[i].className = '';
        }
    }
    else {
        aa.className = "xuanxuanthis";
        var anchors = aa.parentNode.parentNode.childNodes;
        for (i = 0; i < anchors.length; i++) {
...
function showTipsMsg(msg, time, type) {
    if (type == 4) {
        top.showTopMsg(msg, time, 'success'); //头部提示,4、success 5、error 3、warning
    } else if (type == 5) {
        top.showTopMsg(msg, time, 'error'); //头部提示,4、success 5、error 3、warning
    } else if (type == 3) {
        top.showTopMsg(msg, time, 'warning'); //头部提示,4、success 5、error 3、warning
    }
}
...
function IsEditRepayment(id) {
    var isOK = true;
    if (id == undefined || id == "undefined" || id == "") {
        isOK = false;
        showTipsMsg("很抱歉，您当前未选中回款计划！", 4000, 3);
    } else if (id.split(",").length > 1) {
        isOK = false;
        showTipsMsg("很抱歉，一次只能选择一期进行还款操作！", 4000, 3);
    }
    return isOK;
}
...
function IsProjectData(id) {
    var isOK = true;
    if (id == undefined || id == "undefined" || id == "") {
        isOK = false;
        showTipsMsg("很抱歉，您当前未选中项目！", 4000, 3);
    }
    return isOK;
}
...
/*parm：      条件参数
--------------------------------------------------*/
function phCarousel(url, parm, count) {
    Carousel(url, parm, "确定推送轮播", count);
}

function Carousel(url, parm, Msg, count) {
    if (count == undefined) {
        count = 1;
    }
        ...
        if (r) {
            getAjax(url, parm, function (rs) {
                if (rs.toLocaleLowerCase() == 'true') {
                    showTipsMsg("推送成功", 2000, 4);
                    windowload();
                } else {
                    showTipsMsg(rs, 4000, 5);
                    windowload();
                }
            });
        }
...
/*parm：      条件参数
--------------------------------------------------*/
function updateConfig(url, parm, userid, count) {
    UpdateConfig(url, parm, "注：请确认要设置用户：" + userid + "，为黑名单？此操作将无可恢复", count);
}

function UpdateConfig(url, parm, Msg, count) {
    if (count == undefined) {
        count = 1;
    }
    showConfirmMsg(Msg, function (r) {
        if (r) {
            getAjax(url, parm, function (rs) {
                if (rs.toLocaleLowerCase() == 'true') {
                    showTipsMsg("设置成功", 2000, 4);
                    windowload();
                } else if (rs.toLocaleLowerCase() == 'false') {
                    showTipsMsg("修改失败，请稍后重试", 4000, 5);
                } else {
                    showTipsMsg(rs, 4000, 3);
                }
            });
...
/*parm：      条件参数
--------------------------------------------------*/
function updateProject(url, parm, count) {
    UpdateProjectData(url, parm, "注：确认把当前订单设置为完成吗？", count);
}

function UpdateProjectData(url, parm, Msg, count) {
    if (count == undefined) {
        count = 1;
    }
    showConfirmMsg(Msg, function (r) {
...
/*parm：      条件参数
--------------------------------------------------*/
function resendsms(url, parm, count) {
    ResendSms(url, parm, "注：确认重新发送短信吗？", count);
}

function ResendSms(url, parm, Msg, count) {
    if (count == undefined) {
        count = 1;
    }
    showConfirmMsg(Msg, function (r) {
        if (r) {
            getAjax(url, parm, function (rs) {
                if (rs.toLocaleLowerCase() == 'true') {
                    showTipsMsg("重发成功", 2000, 4);
                    windowload();
                } else if (rs.toLocaleLowerCase() == 'false') {
                    showTipsMsg("重发失败，请稍后重试", 4000, 5);
                } else {
                    showTipsMsg(rs, 4000, 3);
                }
            });
...
/*parm：      条件参数
--------------------------------------------------*/
function shielddata(url, parm, count) {
    ShieldData(url, parm, "注：您确定屏蔽当前选中信息吗？", count);
}


function ShieldData(url, parm, Msg, count) {
    if (count == undefined) {
        count = 1;
    }
    showConfirmMsg(Msg, function (r) {
        if (r) {
            getAjax(url, parm, function (rs) {
                if (rs.toLocaleLowerCase() == 'true') {
                    showTipsMsg("屏蔽成功", 2000, 4);
                    windowload();
                } else if (rs.toLocaleLowerCase() == 'false') {
                    showTipsMsg("屏蔽失败，请稍后重试", 4000, 5);
                } else {
                    showTipsMsg(rs, 4000, 3);
                }
            });
...
function Keypress(obj) {
    ...
    /*JQuery 限制文本框只能输入数字*/
    $("#" + obj).keyup(function () {
        var val = parseInt($(this).val().replace(reg, ''));   
        if (isNaN(val)) {
            $(this).val("");
        } else {
            $(this).val(val);
...
function CheckboxValue() {
    ...
    //方法一：
    var reVal = '';
    //            $("[class='xuanxuanthis']").each(function () {
    //                reVal += $(this).attr("dateid") + ",";
    //            });

    //方法二：map
    reVal = $(".xuanxuanthis").map(function () {
        return $(this).attr("dateid")
    }).get().join(',')
...
function treeAttrCss() {
    $('.strTree').lightTreeview({
        ...
        nodeEvent: false, //是否将节点标题的点击也绑定菜单收缩展开的事件。默认为true
...
function treeCss() {
    ...
    $(".strTree li div").live("click", function () {
        if ($(this).attr('class') == "" || $(this).attr('class') == "treeview-folder" || $(this).attr('class') == "treeview-folder treeview-folder-close" || $(this).attr('class') == undefined) {
...
function PQLoadGrid(obj_ID, url, colM, sort, PageSize, topVisible) {
    ...
    var dataModel = {
        ...
        sortIndx: 0,
        sortDir: "up",
        rPPOptions: [10, 15, 20, 30, 40, 50, 100, 500, 1000],
        getUrl: function () {
    ...
    $(obj_ID).pqGrid({
        ...
        rowSelect: function (evt, ui) {
            ...
            if ($("#grid_paging .pq-cont>table").length < 1) {
                var sbhtml = '<div class="nothing" style="padding: 20px;text-align: center;"><img src="/Themes/Images/nothing.jpg"></div>';
                $("#grid_paging .pq-cont").html(sbhtml);
            }
        }
    });
    $(obj_ID).pqGrid("showLoading");
