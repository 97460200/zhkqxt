    ...
    public class WxPayData
    {
        ...
        */
        public SortedDictionary<string, object> FromXml(string xml)
        {
            ...
            try
            {
                //XmlNodeList bodyNode =  .GetElementsByTagName(TagName);//取节点名bodyXmlNode
                ...
                //string AdminHotelid = m_values["AdminHotelid"].ToString();
                //int Hotelid = CommonHelper.GetInt(m_values["Hotelid"]);
                //CheckSign(AdminHotelid, Hotelid);//验证签名,不通过会抛异常
            }
        ...
        */
        ...
        public string MakeSignOwn(string AdminHotelid, int Hotelid, string signType = "MD5")
        {
            //转url格式
            string str = ToUrl();
            //在string后加入API KEY
            str += "&key=" + WxPayConfig.key_own(AdminHotelid, Hotelid);
            if (signType == SIGN_TYPE_MD5)
            {
                var md5 = MD5.Create();
                var bs = md5.ComputeHash(Encoding.UTF8.GetBytes(str));
            ...
            {
                return CalcHMACSHA256Hash(str, WxPayConfig.key_own(AdminHotelid, Hotelid));
            }
            else
            {
                throw new WxPayException("sign_type 不合法");
        ...
        * 正确返回true，错误抛异常
        */
        public bool CheckSign(string AdminHotelid, int Hotelid)
        {
            //如果没有设置签名，则跳过检测
            if (!IsSet("sign"))
            {
            ...
            //在本地计算新的签名
            string cal_sign = MakeSign(AdminHotelid, Hotelid);
