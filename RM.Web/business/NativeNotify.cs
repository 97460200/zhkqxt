...
namespace RM.Web.business
{
    ...
    public class NativeNotify : Notify
    {
        ...
        private WxPayData UnifiedOrder(string openId, string productId, string AdminHotelid, int Hotelid = 0)
        {
            //统一下单
            WxPayData req = new WxPayData();
            req.SetValue("body", "test");
            req.SetValue("attach", "test");
            req.SetValue("out_trade_no", WxPayApi.GenerateOutTradeNo(AdminHotelid, Hotelid));
            req.SetValue("total_fee", 1000000);
            ...
            WxPayData result = WxPayApi.UnifiedOrder(req, AdminHotelid, Hotelid);
            return result;
        }
    }
}