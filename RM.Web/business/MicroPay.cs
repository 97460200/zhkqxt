...
namespace RM.Web.business
{
    public class MicroPay
    {
        ...
        * @return 刷卡支付结果
        */
        public static string Run(string body, string total_fee, string auth_code, string AdminHotelid, int Hotelid)
        {
            ...
            data.SetValue("out_trade_no", WxPayApi.GenerateOutTradeNo(AdminHotelid));//产生随机的商户订单号

            WxPayData result = WxPayApi.Micropay(data, AdminHotelid, 10); //提交被扫支付，接收返回结果
		    ...
		    //签名验证
            result.CheckSign(AdminHotelid, Hotelid);
		    ...
		    while(queryTimes-- > 0)
		    {
                ...
                WxPayData queryResult = Query(out_trade_no, AdminHotelid, Hotelid, out succResult);
			    //如果需要继续查询，则等待2s后继续
			    if(succResult == 2)
                {
				    Thread.Sleep(2000);
            ...
            if (!Cancel(out_trade_no, AdminHotelid))
		    {
	    ...
	    */
        public static WxPayData Query(string out_trade_no, string AdminHotelid, int Hotelid, out int succCode)
	    {
		    WxPayData queryOrderInput = new WxPayData();
		    queryOrderInput.SetValue("out_trade_no",out_trade_no);
            WxPayData result = WxPayApi.OrderQuery(queryOrderInput, AdminHotelid, Hotelid);
	    ...
	    */
        public static bool Cancel(string out_trade_no, string AdminHotelid, int depth = 0)
	    {
		    if(depth > 10)
            {
			    return false;
            ...
            WxPayData result = WxPayApi.Reverse(reverseInput, AdminHotelid);
            ...
            {
                return Cancel(out_trade_no, AdminHotelid, ++depth);
		    }
		    return false;
	    }
    }
