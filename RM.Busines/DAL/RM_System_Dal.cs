...
namespace RM.Busines.DAL
{
    ...
    public class RM_System_Dal : RM_System_IDAO
    {
        ...
        /// </summary>
        /// <returns></returns>
        public DataTable GetMenuBindA()
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append(@"SELECT * FROM ABase_SysMenu WHERE DeleteMark = 1 ORDER BY SortCode ASC");
            return DataFactory.SqlDataBase().GetDataTableBySQL(strSql);
        }
        /// <summary>
        /// 菜单所有列表
        /// </summary>
        /// <returns></returns>
        public DataTable GetMenuBindAgent()
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append(@"SELECT * FROM AgentBase_SysMenu WHERE DeleteMark = 1 ORDER BY SortCode ASC");
            return DataFactory.SqlDataBase().GetDataTableBySQL(strSql);
        }
        /// <summary>
        /// 菜单树列表
        ...
        /// </summary>
        /// <returns></returns>
        public DataTable GetMenuListA()
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append(@"SELECT Menu_Id,Menu_Name,Menu_Img,Menu_Type,TARGET,ParentId,
                            CAST(Menu_Type AS VARCHAR(10)) +'-'+CAST(SortCode AS VARCHAR(10)) AS Sort,
                            NavigateUrl,CreateUserName,CreateDate,ModifyUserName,ModifyDate
                            FROM ABase_SysMenu WHERE DeleteMark = 1 and Menu_Type !=3 ORDER BY SortCode ASC");
            return DataFactory.SqlDataBase().GetDataTableBySQL(strSql);
        }
        ...
        /// </summary>
        /// <returns></returns>
        public DataTable GetMenuListAgent()
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append(@"SELECT Menu_Id,Menu_Name,Menu_Img,Menu_Type,TARGET,ParentId,
                            CAST(Menu_Type AS VARCHAR(10)) +'-'+CAST(SortCode AS VARCHAR(10)) AS Sort,
                            NavigateUrl,CreateUserName,CreateDate,ModifyUserName,ModifyDate
                            FROM AgentBase_SysMenu WHERE DeleteMark = 1 and Menu_Type !=3 ORDER BY SortCode ASC");
            return DataFactory.SqlDataBase().GetDataTableBySQL(strSql);
        }
        ...
        public DataTable GetButtonHtml(string UserId)
        {
            ...
            return DataTableHelper.GetNewDataTable(dt_Menu, "ParentId='" + GetMenuByNavigateUrl(URL, dt_Menu) + "'");
        }
        /// <summary>
        /// 根据菜单URL获取主键
        /// </summary>
        ...
        public string GetMenuByNavigateUrl(string NavigateUrl, DataTable dt_Menu)
        {
            try
            {
                DataTable dt = DataTableHelper.GetNewDataTable(dt_Menu, "NavigateUrl like '%" + NavigateUrl + "%'");
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append(@"SELECT  M.Menu_Type,M.Menu_Id,M.Menu_Name ,M.Menu_Title ,M.Menu_Img ,M.TARGET ,M.ParentId ,M.NavigateUrl ,M.SortCode,M.class
                            FROM    ( SELECT    M.Menu_Type,M.Menu_Name ,M.Menu_Title ,M.Menu_Img ,M.TARGET ,M.ParentId ,M.Menu_Id ,M.NavigateUrl ,M.SortCode ,'角色权限' AS TheirTYPE,M.class
                                      FROM      Base_SysMenu M
                                                LEFT JOIN Base_RoleRight R_R ON R_R.Menu_Id = M.Menu_Id
                                                LEFT JOIN Base_UserRole U_R ON U_R.Roles_ID = R_R.Roles_ID
                                      ...
                                      SELECT    M.Menu_Type,M.Menu_Name ,M.Menu_Title ,M.Menu_Img ,M.TARGET ,M.ParentId ,M.Menu_Id ,M.NavigateUrl ,M.SortCode ,'角色权限' AS TheirTYPE,M.class
                                      FROM      Base_SysMenu M
                                                LEFT JOIN Base_MenuGroupRight M_R ON M_R.Menu_Id = M.Menu_Id
                                                LEFT JOIN Base_RolesMenuGroup R_M ON R_M.MenuGroup_ID = M_R.MenuGroup_ID
                                                LEFT JOIN Base_UserRole U_R ON U_R.Roles_ID = R_M.Roles_ID
                                      WHERE     M.TARGET = 'Iframe' AND U_R.User_ID = @User_ID
                                                AND M.DeleteMark = 1

                                      UNION ALL
                                      SELECT    M.Menu_Type,M.Menu_Name ,M.Menu_Title ,M.Menu_Img ,M.TARGET ,M.ParentId ,M.Menu_Id ,M.NavigateUrl ,M.SortCode ,'用户权限' AS TheirTYPE,M.class
                                      FROM      Base_SysMenu M
                                                LEFT JOIN Base_UserRight U_R ON U_R.Menu_Id = M.Menu_Id
                                      WHERE     M.TARGET = 'Iframe' AND U_R.User_ID = @User_ID
                                                AND M.DeleteMark = 1
                                    ) M
                            GROUP BY M.Menu_Type,M.Menu_Id ,M.Menu_Name ,M.Menu_Title ,M.Menu_Img ,M.TARGET ,M.ParentId ,M.NavigateUrl ,M.SortCode,m.class ORDER BY M.SortCode");
            SqlParam[] para = {
                                         new SqlParam("@User_ID",UserId)};
            return DataFactory.SqlDataBase().GetDataTableBySQL(strSql, para);
        }
        ...
        /// <param name="UserId">用户主键</param>
        /// <returns></returns>
        public DataTable GetMenuHtmlA(string UserId)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append(@"SELECT  M.Menu_Type,M.Menu_Id,M.Menu_Name ,M.Menu_Title ,M.Menu_Img ,M.TARGET ,M.ParentId ,M.NavigateUrl ,M.SortCode,M.class
                            FROM    ( SELECT    M.Menu_Type,M.Menu_Name ,M.Menu_Title ,M.Menu_Img ,M.TARGET ,M.ParentId ,M.Menu_Id ,M.NavigateUrl ,M.SortCode ,'角色权限' AS TheirTYPE,M.class
                                      FROM      ABase_SysMenu M
                                                LEFT JOIN ABase_RoleRight R_R ON R_R.Menu_Id = M.Menu_Id
                                                LEFT JOIN ABase_UserRole U_R ON U_R.Roles_ID = R_R.Roles_ID
                                      WHERE     M.TARGET = 'Iframe' AND U_R.User_ID = @User_ID
                                                AND M.DeleteMark = 1 
                                      ...
                                      SELECT    M.Menu_Type,M.Menu_Name ,M.Menu_Title ,M.Menu_Img ,M.TARGET ,M.ParentId ,M.Menu_Id ,M.NavigateUrl ,M.SortCode ,'用户权限' AS TheirTYPE,M.class
                                      FROM      ABase_SysMenu M
                                                LEFT JOIN ABase_UserRight U_R ON U_R.Menu_Id = M.Menu_Id
                                      WHERE     M.TARGET = 'Iframe' AND U_R.User_ID = @User_ID
                                                AND M.DeleteMark = 1
                                    ) M
                            GROUP BY M.Menu_Type,M.Menu_Id ,M.Menu_Name ,M.Menu_Title ,M.Menu_Img ,M.TARGET ,M.ParentId ,M.NavigateUrl ,M.SortCode,m.class ORDER BY M.SortCode");
        ...
        /// <param name="UserId">用户主键</param>
        /// <returns></returns>
        public DataTable GetMenuHtmlAgent(string UserId)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append(@"SELECT  M.Menu_Type,M.Menu_Id,M.Menu_Name ,M.Menu_Title ,M.Menu_Img ,M.TARGET ,M.ParentId ,M.NavigateUrl ,M.SortCode,M.class
                            FROM    ( SELECT    M.Menu_Type,M.Menu_Name ,M.Menu_Title ,M.Menu_Img ,M.TARGET ,M.ParentId ,M.Menu_Id ,M.NavigateUrl ,M.SortCode ,'角色权限' AS TheirTYPE,M.class
                                      FROM      AgentBase_SysMenu M
                                                LEFT JOIN AgentBase_RoleRight R_R ON R_R.Menu_Id = M.Menu_Id
                                                LEFT JOIN AgentBase_UserRole U_R ON U_R.Roles_ID = R_R.Roles_ID
                                      WHERE     M.TARGET = 'Iframe' AND U_R.User_ID = @User_ID
                                                AND M.DeleteMark = 1 
                                      ...
                                      SELECT    M.Menu_Type,M.Menu_Name ,M.Menu_Title ,M.Menu_Img ,M.TARGET ,M.ParentId ,M.Menu_Id ,M.NavigateUrl ,M.SortCode ,'用户权限' AS TheirTYPE,M.class
                                      FROM      AgentBase_SysMenu M
                                                LEFT JOIN AgentBase_UserRight U_R ON U_R.Menu_Id = M.Menu_Id
                                      WHERE     M.TARGET = 'Iframe' AND U_R.User_ID = @User_ID
                                                AND M.DeleteMark = 1
                                    ) M
                            GROUP BY M.Menu_Type,M.Menu_Id ,M.Menu_Name ,M.Menu_Title ,M.Menu_Img ,M.TARGET ,M.ParentId ,M.NavigateUrl ,M.SortCode,m.class ORDER BY M.SortCode");
        ...
        /// <returns></returns>
        public string GetNumberByTableName(string tbName, string iLength)
        {
            string retVal = "";
            StringBuilder sbPi = new StringBuilder();
            sbPi.Append("SELECT COUNT(*) FROM  " + tbName);
            DataTable dt = DataFactory.SqlDataBase().GetDataTableBySQL(sbPi);
            int count = 0;
            if (dt != null && dt.Rows.Count > 0)
            {
                count = Convert.ToInt32(dt.Rows[0][0]);
            }
            if (count.ToString().Length >= iLength.Length)
            {
                retVal = count.ToString();
            }
            else
            {
                int l = iLength.ToString().Length - count.ToString().Length;
                retVal = iLength.Substring(0, l) + count.ToString();
            }
            return retVal;
        }
